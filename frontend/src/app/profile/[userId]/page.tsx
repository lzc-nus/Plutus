...
export default function ProfilePage() {
  ...
  const [posts, setPosts] = useState<import("@/lib/api/generated").PostRead[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      ...
      try {
        const [profileResponse, followersResponse, followingResponse, postCountResponse] = 
          await Promise.all([
            getUserById(userId),
            getFollowers(userId),
            getFollowing(userId),
  ...
  useEffect(() => {
    if (viewerLoading || viewer === null || followerCount === null) {
      return;
    }
    ...
    getFollowers(userId)
      .then((res) => {
        ...
        setIsFollowing(
          followers.some(
            follower => follower.follower_id === viewer.id
          )
        );
      });
  }, [userId, viewer, viewerLoading, followerCount]);
  ...
  useEffect(() => {
    ...
    async function loadPosts() { 
      const response = await getUserPosts(userId, 100); 
    ...
    return (
      <main className="min-h-screen bg-[#f4efe6] px-4 py-12 sm:px-6 lg:px-10">
        <section className="mx-auto max-w-xl rounded-lg border border-[#d7c6a3]/45 bg-[#fbf7ef] px-6 py-12 text-center shadow-[0_18px_48px_rgba(43,34,24,0.08)]">
          <p className="text-base font-bold text-[#1c2018]">
  ...
  return (
    <main className="min-h-screen bg-[#f4efe6] px-4 pt-4 pb-8 sm:px-6 lg:px-10">
      <div 
        className={`mx-auto grid max-w-[1180px] items-start gap-6 transition-all duration-300 ${
          sidebarCollapsed 
          ...
          <section className="overflow-hidden rounded-lg border border-[#d7c6a3]/45 bg-[#fbf7ef] shadow-[0_18px_48px_rgba(43,34,24,0.08)] lg:sticky lg:top-28">
            ...
            <div className="px-5 pb-5">
              ...
              <div className="mt-4 min-w-0">
                <h1 className="truncate text-2xl font-bold text-[#1c2018]">
                  {displayName}
                </h1>
        ...
        <div className="flex flex-col gap-4 min-w-0 pt-0">
          ...
          <section className="min-w-0 overflow-hidden rounded-lg border border-[#d7c6a3]/45 bg-[#fbf7ef] shadow-[0_18px_48px_rgba(43,34,24,0.08)]">
            <header className="border-b border-[#d7c6a3]/35 bg-[#fbf7ef]/95 px-4 py-4 backdrop-blur sm:px-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-1 text-xl font-bold text-[#1c2018]">
                    Posts
                  </h2>
                </div>
            ...
            {postCount === 0 ? (
              ...
              <PostFeed
                fetcher={fetchPosts}
                feedKey={`profile-${userId}-${viewer?.id ?? "guest"}`}
                viewer={viewer}
                onAuthRequired={setAuthAction}
...
function ProfileStat({
  ...
  return (
    ...
    >
      ...
      <span className="mt-1 block text-xs font-semibold text-[#8a7c65]">
        {label}
      </span>
    </Link>
  );
...
function EditProfileModal({ user, onClose, onSaved }: EditProfileModalProps) {
  ...
  async function handleSave(data: ProfileFormInput) {
  ...
  return (
    ...
    >
      <div className="flex w-full max-w-xl flex-col rounded-t-lg border border-[#d7c6a3]/45 bg-[#fbf7ef] shadow-[0_24px_80px_rgba(28,32,24,0.24)] sm:rounded-lg">
        <div className="flex items-center justify-between border-b border-[#d7c6a3]/35 px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[#7c7468]">
              Update your public community identity.
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit(handleSave)} className="grid gap-4 px-5 py-5">
          <ProfileField 
            label="Display name" 
      ...
      <div className="mx-auto grid max-w-[1180px] gap-6 lg:grid-cols-[340px_minmax(0,1fr)]">
        ...
        <section className="animate-pulse rounded-lg border border-[#d7c6a3]/45 bg-[#fbf7ef]">
          ...
          {Array.from({ length: 4 }).map((_, index) => (
            <div 
              key={index} 
              className="border-b border-[#d7c6a3]/30 px-4 py-4"
            >
