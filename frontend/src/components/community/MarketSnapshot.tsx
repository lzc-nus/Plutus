...
function loadWatchlist(): string[] {
  try {
    ...
    return normalizeWatchlist(
      stored ? JSON.parse(stored) : DEFAULT_SYMBOLS
    );
  } catch {
...
export function MarketSnapshot() {
  ...
  useEffect(() => {
    ...
    async function syncWatchlist() {
      ...
      try {
        ...
        if (serverSymbols.length === 0) {
          ...
          syncToServer(localSymbols).catch(() => {});
          setWatchlist(localSymbols);
        } else {
    ...
    window.addEventListener("plutus-auth-refresh", syncWatchlist);

    return () => window.removeEventListener("plutus-auth-refresh", syncWatchlist);
  ...
  const loadQuotes = useCallback(async (symbols: string[]) => {
    ...
    try {
      ...
      // Don't let an older request overwrite newer data.
      if (requestId !== quotesRequestId.current || !mountedRef.current) {
        return;
      }
  ...
  function handleAddSymbol() {
    ...
    addWatchlistSymbol(symbol).catch(error => 
      console.error("addWatchlistSymbol failed", error));
  }
