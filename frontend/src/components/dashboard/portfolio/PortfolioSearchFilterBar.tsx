...
function countActiveFilters(filters: PortfolioOverviewFilterState): number {
  let count = 0;
  if (filters.assetCategories.length > 0) count += 1;
  if (filters.liabilityCategories.length > 0) count += 1;
  if (filters.valueMin || filters.valueMax) count += 1;
  return count;
}

/**
 * Search/filter bar for the /portfolio overview page. Since this page shows
 * aggregated category cards rather than individual items, this filters which
 * category rows/cards are visible within the Assets and Liabilities summary
 * cards — it does not search individual line items (that's done on the
 * /portfolio/assets and /portfolio/liabilities subpages).
 */
export function PortfolioSearchFilterBar({ filters, onChange }: PortfolioSearchFilterBarProps) {
  const [panelOpen, setPanelOpen] = useState(false);
  const activeCount = countActiveFilters(filters);

  function toggleAssetCategory(cat: AssetCategory) {
    const next = filters.assetCategories.includes(cat)
      ? filters.assetCategories.filter((c) => c !== cat)
      : [...filters.assetCategories, cat];
    onChange({ ...filters, assetCategories: next });
  }

  function toggleLiabilityCategory(cat: LiabilityCategory) {
    const next = filters.liabilityCategories.includes(cat)
      ? filters.liabilityCategories.filter((c) => c !== cat)
      : [...filters.liabilityCategories, cat];
    onChange({ ...filters, liabilityCategories: next });
  }
    ...
    <div className="rounded-xl border border-[#d9d0c1] bg-[#fbf7ef] shadow-[0_2px_12px_rgba(43,34,24,0.05)]">
      ...
      {panelOpen && (
        <div className="space-y-4 border-t border-[#e4dece] p-4">
          ...
          {/* Value range — applies to both assets and liabilities totals */}
          <div>
            <p className="mb-2 text-xs font-medium uppercase tracking-[0.1em] text-[#9a8f7a]">
              Category total range (USD)
            </p>
